...
import { parentConversations } from '@/data/dashboardData'
...
export default function MessageTeachers() {
  const [conversations, setConversations] = useState<Conversation[]>(
    parentConversations as Conversation[],
  )
    ...
    setConversations((prev) =>
      prev.map((c) =>
        c.id === activeId
          ? {
              ...c,
              messages: [...c.messages, { id: Date.now(), from: 'parent', text: draft.trim(), time: 'Just now' }],
            }
          : c,
      ),
    )
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-lg font-semibold text-ink-900">Message Teachers</h2>
        <p className="mt-1 text-sm text-ink-500">Direct messaging with staff.</p>
      </div>
      ...
      <div className="grid overflow-hidden rounded-2xl border border-ink-100 bg-paper lg:grid-cols-[280px_1fr]">
          ...
          {conversations.map((c) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-ink-900">{c.teacher}</p>
                <p className="mt-0.5 truncate text-xs text-ink-400">{c.subject}</p>
        ...
        <div className="flex flex-col">
          <div className="border-b border-ink-100 px-6 py-4">
            <p className="font-display text-base font-semibold text-ink-900">{active.teacher}</p>
            <p className="text-xs text-ink-400">{active.subject}</p>
          </div>
                ...
                >
                  ...
                  <p
                    className={`mt-1 text-[10px] ${
                      m.from === 'parent' ? 'text-ink-300' : 'text-ink-400'
                    }`}
                  >
                    {m.time}
                  </p>
